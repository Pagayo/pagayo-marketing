...
- **Styling:** CSS-variabelen uit `:root` in `global.css`; sectie-specifieke layout in component-`<style>` alleen als het pagina-uniek is (zie `features.astro`).
...
5. **Secties** — nieuwe blokken in `components/sections/`; styling in `global.css` of beperkt scoped `<style>`.
